...
### More features
- Google Analytics built in (replace `UA-xxxxxxxx-x` with your personal analytics verification key in `_includes/2024_data/head.html`)
- SEO (check `_config.yml`)
- Customized 404 Page Not Found Page
- Designed to be futureproof as you can create a subpages for each year (eg. YOUR-LINK.github.io/2015, YOUR-LINK.github.io/2024, etc)
...
## Files Structure
- The main stylesheet is stored `/css/2024_style`. Both `2024` and `2025` are sharing the same stylesheet.
- In `/css/2024_style/img` you will find where the pictures in `/2024` are stored at. You will find the images of speakers in `/css/2025_style/img`. This setup is to ensure that we can easily to move from year to year by creating new folders.
- `_2024_pages` and `_2025_pages` are the folders that store subpages.
- `_2024_data` and `_2025_data` are the folders that store each sections in home page. Those sections are can be removed by removing or commenting out the `include` code in `_layout/2024_home.html` or `_layout/2025_home.html`.
- `_data/twenty_24/` and `_data/twenty_25/` contains `data` files for agenda, faqs, home about section data, speakers, and team members information. The data file is in the format of `.yml`. The reason why the folders are named `twenty_24` and `twenty_25` are due to liquid syntax will throw errors if the name contains integer. 
